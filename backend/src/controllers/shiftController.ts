...
const objectId = (field: string) =>
  z
    .string()
    .refine((value) => mongoose.Types.ObjectId.isValid(value), `${field} must be a valid ObjectId`);
...
function getShiftDateTimes(
  ...
  const endsAt =
    crossesMidnight || sameDayEndsAt <= startsAt
...
const createSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      ...
      data.definitionId !== data.shiftDefinitionId
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
const updateSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      data.requiredCount &&
      data.requiredStaffCount &&
      data.requiredCount !== data.requiredStaffCount
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
async function attachTemplateStatusToShift(shift: unknown): Promise<ShiftResponse> {
  ...
  const definition = definitionId ? await ShiftDefinition.findById(definitionId).lean() : null;
...
async function attachTemplateStatusToShifts(shifts: unknown[]): Promise<ShiftResponse[]> {
  ...
  const definitionIds = [
    ...new Set(shiftObjects.map((shift) => String(shift.definitionId)).filter(Boolean)),
  ];
  ...
  return shiftObjects.map((shift) => {
    ...
    return {
      ...shift,
      templateStatus: isModifiedFromDefinition(
        shift,
        definitionById.get(String(shift.definitionId))
      )
        ? 'manually_modified'
        : 'matching_template',
    };
...
function logShiftValidationError(
...
export async function updateShift(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    let crossesMidnight =
      before.endsAt.getTime() > buildDateTime(before.date, before.endTime).getTime();
    ...
    if (parsed.data.startTime && !parsed.data.definitionId)
      update.startTime = parsed.data.startTime;
