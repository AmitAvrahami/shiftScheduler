...
export default function AdminWeeklyStaffingPage() {
  ...
  // Fetch shifts + schedule status on weekId change
  ...
  // Group shifts by date key, sorted by startTime per day
    ...
    <MainLayout title="תקן משמרות שבועי" subtitle={<WeekLabel weekId={weekId} />}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-full border border-slate-200">
          <button
            onClick={() => setWeekId((w) => getPrevWeekId(w))}
            className="w-8 h-8 rounded-full flex items-center justify-center text-slate-600 hover:bg-white hover:shadow-sm transition-all"
            aria-label="שבוע קודם"
          >
            <MaterialIcon name="chevron_right" />
          </button>
          <span className="text-sm font-bold text-slate-700 px-2">
            שבוע {weekId.split('-W')[1]}
          </span>
          <button
            onClick={() => setWeekId((w) => getNextWeekId(w))}
            className="w-8 h-8 rounded-full flex items-center justify-center text-slate-600 hover:bg-white hover:shadow-sm transition-all"
            aria-label="שבוע הבא"
          >
        ...
        {scheduleStatus && (
          <span className="text-xs font-semibold px-3 py-1 rounded-full border bg-slate-50 text-slate-600 border-slate-200">
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-6 z-50 px-4 py-3 rounded-xl shadow-lg text-sm font-semibold flex items-center gap-2 ${
