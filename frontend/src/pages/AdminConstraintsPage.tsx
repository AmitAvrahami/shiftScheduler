      ...
      <div className="flex flex-col gap-6" dir="rtl">
        <div className="bg-white p-6 shadow-sm rounded-xl border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-wrap items-center gap-4 order-2 md:order-1">
            {/* Lock state is server-derived — render only once loaded so it never
                flashes a wrong default (open/unlocked) before data arrives. */}
            {!loading && !loadError && isLocked !== null && (
              <div
              ...
              >
                <div
                  className={`w-3 h-3 rounded-full ${isLocked ? 'bg-red-600' : 'bg-green-600'}`}
                />
