      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="grid gap-4">
          {definitions.map((def) => (
            <div
              key={def._id}
              className="bg-white p-4 rounded shadow flex justify-between items-center border-l-4"
              style={{ borderColor: def.color }}
            >
              <div>
                <h3 className="font-bold text-lg">{def.name}</h3>
                <p className="text-gray-600">
                  {def.startTime} - {def.endTime} ({def.durationMinutes} mins)
                </p>
              </div>
